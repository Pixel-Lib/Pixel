...
void Odom::calibrate(bool calibrateIMUs) {
  ...
  for (auto it = this->horizontals.begin(); it != this->horizontals.end();
    ...
    if (pxl::sgn((*it)->getOffset() == 1))
      std::cout << "Left drivetrain motor failed to calibrate!" << std::endl;
  }
...
void Odom::update() {
  ...
  if (this->imu.size() > 0) {
    ...
    theta +=
        Odom::calcDeltaTheta(*this->verticals.at(0), *this->verticals.at(1));
  } else if (drivetrain.size() > 1) {
    theta +=
        Odom::calcDeltaTheta(*this->drivetrain.at(0), *this->drivetrain.at(1));
  } else {
    std::cerr
        << "Odom calculation failure! Not enough sensors to calculate heading"
        << std::endl;
    return;
  }
