...
namespace pxl {
  ...
  //     imu(imu) {}
pxl::Drivetrain::Drivetrain(pros::MotorGroup *leftMotors, pros::MotorGroup *rightMotors,
    ...
    if (isDriverControl() && !calibrated) {
        sensors.imu = nullptr;
        std::cerr<<
            "Driver control started, abandoning IMU calibration, defaulting to tracking wheels / motor encoders"<<std::endl;
    }
