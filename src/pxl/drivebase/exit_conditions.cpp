...
namespace pxl {
 ...
 bool stand_still::updateStandstill(movement_Type type, bool& standStill, float error, float lastError, uint8_t& standStillCount){
    if (type == lateral) {
    if (SSActive && fabs(lastError - error) <= maxStepDistance_t) {
      standStillCount++;
      if (standStillCount > SSMaxCount_lateral) {
        return standStill = true;
      }
    } 
    ...
    if (SSActive_t && fabs(lastError - error) <= maxStepTurn_d) {
         standStillCount++;
     if (standStillCount > SSMaxCount_turn) {
        return standStill = true;
      }
    } 
    else { standStillCount = 0;}
