...
TrackingWheel::TrackingWheel(pros::MotorGroup *motors, float wheelDiameter,
...
float TrackingWheel::getDistanceTraveled() {
    if (this->encoder != nullptr)
        return (float(this->encoder->get_value()) * this->diameter * M_PI /
                360) /
               this->gearRatio;
        ...
        std::transform(positions.begin(), positions.end(), gearsets.begin(),
                       std::back_inserter(distances),
                       [&](double pos, pros::motor_gearset_e_t gearset) {
                           float in =
                               (gearset == pros::E_MOTOR_GEARSET_36)   ? 100
