...
class Motor {
     ...
     * move_absolute(0) or move_relative(0).
     *
     * This function uses the following values of errno when an error state is
     * reached:
     * ENODEV - The port cannot be configured as a motor
     ...
     * (motor_move_absolute() or motor_move_relative()). This will have no
    ...
    /**
     * Takes in floating point values and returns a properly formatted pid
     * struct. The motor_pid_s_t struct is in 4.4 format, i.e. 0x20 is 2.0, 0x21
     * is 2.0625, etc. This function will convert the floating point values to
     * the nearest 4.4 value.
     *
     * \param kf
     ...
     */
    [[deprecated(
        "Changing these values is not supported by VEX and may lead to "
     ...
     *        The integral limit
     * \param threshold
     *        The threshold for determining if a position movement has reached
     * its goal. This has no effect for velocity PID calculations. \param
    ...
    /**
     * Gets the position PID that was set for the motor. This function will
     * return zero for all of the parameters if the motor_set_pos_pid() or
     ...
     *
     * \return A motor_pid_full_s_t containing the position PID constants last
     * set to the given motor
     */
    [[deprecated(
        "Changing these values is not supported by VEX and may lead to "
        "permanent motor "
    ...
    /**
     * Gets the velocity PID that was set for the motor. This function will
     * return zero for all of the parameters if the motor_set_vel_pid() or
     ...
     * to their negative maximum values.
     *
     * \return A motor_pid_full_s_t containing the velocity PID constants last
     * set to the given motor
     */
    [[deprecated(
        "Changing these values is not supported by VEX and may lead to "
...
class Motor_Group {
     ...
     *
     * This movement is relative to the position of the motors when initialized
     * or the position when it was most recently reset with
     * pros::Motor::set_zero_position().
     *
    ...
    /**
     * Sets the output voltage for the motors from -12000 to 12000 in
     * millivolts.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     ...
     * EACCESS - The Motor group mutex can't be taken or given
     *
     * \return A vector with the each motor's actual velocity in RPM in the
     * order or a vector filled with PROS_ERR_F if the operation failed, setting
     * errno.
     */
     ...
     *  +-100, +-200, or +-600, or a vector filled with PROS_ERR if the
     * operation failed, setting errno.
     */
     ...
     * or a vector filled with PROS_ERR_F if the operation failed, setting
     * errno.
     */
     ...
     * EACCESS - The Motor group mutex can't be taken or given
     *
     * \return A vector filled with one of motor_encoder_units_e_t for each
     * motor according to what is set for the motor or a vector filled with
