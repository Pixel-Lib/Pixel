...
class SplineGenerator {
    ...
    public:
        ...
        struct GeneratedPoint {
                GeneratedPoint(Pose ipose, double icurvature = 0.0) : pose(ipose), curvature(icurvature) {}
        ...
        struct GeneratedVector {
                ...
                std::string to_string() const {
                    return "GeneratedVector: {" + point.to_string() + ", vel: " + std::to_string(vel)
                           + ", accel: " + std::to_string(accel) + ", jerk: " + std::to_string(jerk) + "}";
                }
        };

        std::vector<GeneratedVector> gen_single_raw_path(ControlVector start, ControlVector end, int duration,
        ...
        struct ConstrainedState {
                ConstrainedState(Pose ipose, double icurvature, double idistance, double imax_vel, double imin_accel,
                                 double imax_accel)
                    : pose(ipose),
                ...
                std::string to_string() const {
                    return "ConstrainedState: {x: " + std::to_string(pose.x) + ", y: " + std::to_string(pose.y)
                           + ", yaw: " + std::to_string(pose.yaw) + ", k: " + std::to_string(curvature)
                           + ", dist: " + std::to_string(distance) + ", v: " + std::to_string(max_vel)
                           + ", min_a: " + std::to_string(min_accel) + ", max_a: " + std::to_string(max_accel) + "}";
                }
        };
         ...
         */
        template <class Iter> std::vector<ProfilePoint> _generate(Iter start, Iter end, bool fast);
    ...
    public:
         ...
         */
        std::vector<ProfilePoint> parameterize(const ControlVector start, const ControlVector end,
         ...
         */
        ProfilePoint get_point_at_time(const ControlVector start, const ControlVector end,
                                       std::vector<ProfilePoint> points, double t);

        /**
         * Linearly interpolates between points along the profiled curve.
         */
        ProfilePoint lerp_point(QuinticPolynomial x_qp, QuinticPolynomial y_qp, ProfilePoint start, ProfilePoint end,
                                double i);
