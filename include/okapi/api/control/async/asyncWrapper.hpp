...
namespace okapi {
template <typename Input, typename Output> class AsyncWrapper : virtual public AsyncController<Input, Output> {
    public:
        /**
         ...
         */
        void controllerSet(const Input ivalue) override { controller->controllerSet(ivalue); }
         ...
         */
        Input getProcessValue() const override { return controller->getProcessValue(); }
         ...
         */
        bool isSettled() override { return isDisabled() || controller->isSettled(); }
         ...
         */
        void setSampleTime(const QTime &isampleTime) { controller->setSampleTime(isampleTime); }
         ...
         */
        void setOutputLimits(const Output imax, const Output imin) { controller->setOutputLimits(imax, imin); }
         ...
         */
        void startThread() {
            if (!task) { task = new CrossplatformThread(trampoline, this, "AsyncWrapper"); }
        }
    ...
    protected:
        ...
        void loop() {
            ...
            while (!dtorCalled.load(std::memory_order_acquire) && !task->notifyTake(0)) {
                if (!isDisabled()) { output->controllerSet(controller->step(input->controllerGet())); }
