...
class CrossplatformThread {
    public:
#ifdef THREADS_STD
        CrossplatformThread(void (*ptr)(void *), void *params, const char *const = "OkapiLibCrossplatformTask")
#else
        ...
        ~CrossplatformThread() {
            ...
            thread.join();
#else
            if (pros::c::task_get_state(thread) != pros::E_TASK_STATE_DELETED) { pros::c::task_delete(thread); }
#endif
        }
...
#else
        std::uint32_t notifyTake(const std::uint32_t itimeout) { return pros::c::task_notify_take(true, itimeout); }
#endif
