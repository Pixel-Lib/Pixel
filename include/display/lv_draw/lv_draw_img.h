...
typedef struct {
     ...
     * types. For more info see `lv_img_get_src_type()` in lv_img.c */
...
enum {
    ...
    LV_IMG_CF_TRUE_COLOR, /*Color format and depth should match with LV_COLOR
                             settings*/
    LV_IMG_CF_TRUE_COLOR_ALPHA, /*Same as `LV_IMG_CF_TRUE_COLOR` but every pixel
                                   has an alpha byte*/
    ...
    LV_IMG_CF_INDEXED_4BIT, /*Can have 16 different colors in a palette (always
                               chroma keyed)*/
    LV_IMG_CF_INDEXED_8BIT, /*Can have 256 different colors in a palette (always
                               chroma keyed)*/
 ...
 */
typedef lv_res_t (*lv_img_decoder_info_f_t)(const void *src,
                                            lv_img_header_t *header);
 ...
 * @param src the image source. Can be a pointer to a C array or a file name
 * (Use `lv_img_src_get_type` to determine the type)
 * @param style the style of image (maybe it will be required to determine a
 * color or something)
 * @return there are 3 possible return values:
 *    1) buffer with the decoded image
 *    2) if can decode the whole image NULL. decoder_read_line will be called to
 * read the image line-by-line 3) LV_IMG_DECODER_OPEN_FAIL if the image format
 * is unknown to the decoder or an error occurred
 */
typedef const uint8_t *(*lv_img_decoder_open_f_t)(const void *src,
 ...
 */
typedef lv_res_t (*lv_img_decoder_read_line_f_t)(lv_coord_t x, lv_coord_t y,
...
/**
 * Set custom decoder functions. See the typdefs of the function typed above for
 ...
 */
void lv_img_decoder_set_custom(lv_img_decoder_info_f_t info_fp,
                               lv_img_decoder_open_f_t open_fp,
