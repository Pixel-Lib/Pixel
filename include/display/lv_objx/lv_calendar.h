 ...
 * @param calendar pointer to a calendar object
 * @param today pointer to an `lv_calendar_date_t` variable containing the date
 * of today. The value will be saved it can be local variable too.
 */
 ...
 * days[7] = {"Sun", "Mon", ...}` Only the pointer will be saved so this
 ...
 * @param calendar pointer to a calendar object
 * @param day_names pointer to an array with the names. E.g. `const char *
 * days[12] = {"Jan", "Feb", ...}` Only the pointer will be saved so this
 * variable can't be local which will be destroyed later.
 */
