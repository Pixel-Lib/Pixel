...
/**
 * Enable the scroll propagation feature. If enabled then the page will move its
 * parent if there is no more space to scroll.
...
/**
 * Get the the scrolling with arrows (LV_GROUP_KEY_LEFT/RIGHT/UP/DOWN) is
 * enabled or not
 * @param page pointer to a page object
...
/**
 * Get that height which can be set to the children to still not cause overflow
 * (show scrollbars)
 * @param page pointer to a page object
 ...
 * internally. Start an edge flash animation. Exactly one
 * `ext->edge_flash.xxx_ip` should be set
 * @param page
 */
