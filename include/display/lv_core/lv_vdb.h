...
#define LV_VDB_PX_BPP LV_COLOR_SIZE /* Default is LV_COLOR_SIZE */
    ...
    (LV_VDB_SIZE != LV_HOR_RES * LV_VER_RES || LV_VDB_DOUBLE == 0)
#error                                                                         \
    "With LV_VDB_TRUE_DOUBLE_BUFFERED: (LV_VDB_SIZE = LV_HOR_RES * LV_VER_RES and LV_VDB_DOUBLE = 1 is required"
#endif

/* The size of VDB in bytes.
 * (LV_VDB_SIZE * LV_VDB_PX_BPP) >> 3): just divide by 8 to convert bits to
 * bytes
...
/**
 * Get currently active VDB, where the drawing happens. Used with `LV_VDB_DOUBLE
 * 1`
...
/**
 * Get currently inactive VDB, which is being displayed or being flushed. Used
 * with `LV_VDB_DOUBLE  1`
 * @return pointer to the inactive VDB. If `LV_VDB_DOUBLE  0` give the single
 * VDB
 */
