 ...
 */
    ...
    private:
        ...
        float calcDeltaTheta(std::vector<std::shared_ptr<pros::IMU>> &imu, bool update = true);  // Calculates the change in angle using IMUs
        float calcDeltaTheta(TrackingWheel &tracker1, TrackingWheel &tracker2);  // Calculates the change in angle using tracking wheels
};
