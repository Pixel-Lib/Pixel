...
class Timer {
    ...
    /**
     * The start() function sets the start time to the current time in milliseconds using `pros/rtos`.
     */
    void start();
     ...
     *
     * @param start_time Optional` parameter to define the start time of the timer.
     */
    void stop(uint32_t start_time = -1);

    /**
     * `get_elapsed_time` calculates the elapsed time since the timer was started in
     * milliseconds.
     *
     * @return the elapsed time in milliseconds since the timer
     * was started. If the timer has not been started (when `start_time` is -1), it returns -1 to indicate
     * that the timer is not running.
     */
     ...
     * time set in the `stop_time` variable. If the `stop_time` is not set (i.e., it is -1), the function
